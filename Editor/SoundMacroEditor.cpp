...
SoundMacroEditor* FieldSoundMacroStep::getEditor() const {
  return qobject_cast<SoundMacroEditor*>(
      parentWidget()->parentWidget()->parentWidget()->parentWidget()->parentWidget()->parentWidget()->parentWidget());
}

SoundMacroListing* FieldSoundMacroStep::getListing() const {
  return qobject_cast<SoundMacroListing*>(parentWidget()->parentWidget()->parentWidget());
}
...
void FieldSoundMacroStep::updateMacroField() {
  if (!m_macroField) {
    int numCmds = int(static_cast<ProjectModel::SoundMacroNode*>(getListing()->currentNode())->m_obj->m_cmds.size());
...
CommandWidget::CommandWidget(QWidget* parent, amuse::SoundMacro::ICmd* cmd, amuse::SoundMacro::CmdOp op,
                             SoundMacroListing* listing)
: QWidget(parent)
, m_titleLabel(this)
, m_deleteButton(this)
, m_cmd(cmd)
    ...
    for (int f = 0; f < 7; ++f) {
      ...
      if (!field.m_name.empty()) {
        ...
        case amuse::SoundMacro::CmdIntrospection::Field::Type::SampleId: {
          ...
          auto* collection =
              g_MainWindow->projectModel()->getGroupNode(listing->currentNode())->getCollectionOfType(collectionType);
          ...
          const int index = collection->indexOfId(
              amuse::AccessField<amuse::SoundMacroIdDNA<amuse::Endian::Little>>(m_cmd, field).id);
        ...
        case amuse::SoundMacro::CmdIntrospection::Field::Type::SoundMacroStep: {
          ...
          sb->m_spinBox.setValue(
              amuse::AccessField<amuse::SoundMacroStepDNA<amuse::Endian::Little>>(m_cmd, field).step);
...
public:
  ValChangedUndoCommand(amuse::SoundMacro::ICmd* cmd, const QString& fieldName,
                        const amuse::SoundMacro::CmdIntrospection::Field& field, int redoVal,
                        amuse::ObjToken<ProjectModel::SoundMacroNode> node)
  ...
  void undo() override {
    ...
    case amuse::SoundMacro::CmdIntrospection::Field::Type::TableId:
    case amuse::SoundMacro::CmdIntrospection::Field::Type::SampleId:
      amuse::AccessField<amuse::SoundMacroIdDNA<amuse::Endian::Little>>(m_cmd, m_field).id = uint16_t(m_undoVal);
      break;
    default:
      break;
    }
  ...
  void redo() override {
    ...
    case amuse::SoundMacro::CmdIntrospection::Field::Type::TableId:
    case amuse::SoundMacro::CmdIntrospection::Field::Type::SampleId:
      m_undoVal = amuse::AccessField<amuse::SoundMacroIdDNA<amuse::Endian::Little>>(m_cmd, m_field).id.id;
      amuse::AccessField<amuse::SoundMacroIdDNA<amuse::Endian::Little>>(m_cmd, m_field).id = uint16_t(m_redoVal);
      break;
    default:
      break;
    }
...
void CommandWidget::boolChanged(int state) {
  if (m_introspection) {
    ...
    g_MainWindow->pushUndoCommand(new ValChangedUndoCommand(m_cmd, sender()->property("fieldName").toString(), field,
...
void CommandWidget::nodeChanged(int value) {
  ...
  if (m_introspection) {
    ...
    g_MainWindow->pushUndoCommand(
        new ValChangedUndoCommand(m_cmd, fieldW->property("fieldName").toString(), field, v, getParent()->m_node));
  }
}
...
void CommandWidgetContainer::animationDestroyed() { m_animation = nullptr; }
...
void SoundMacroListing::_moveDrag(int hoverIdx, const QPoint& pt, QWidget* source, QMouseEvent* event) {
  ...
  if (hoverIdx != m_dragOpenIdx) {
    ...
    if (CommandWidgetContainer* nextItem = qobject_cast<CommandWidgetContainer*>(m_layout->itemAt(hoverIdx)->widget()))
...
void SoundMacroListing::insert(amuse::SoundMacro::CmdOp op, const QString& text) {
  ...
  m_layout->insertWidget(insertIdx, new CommandWidgetContainer(this, m_node->m_obj->insertNewCmd(insertIdx, op), this));
...
void SoundMacroListing::reindex() {
  for (int i = 0; i < m_layout->count() - 1; ++i)
    if (CommandWidgetContainer* item = qobject_cast<CommandWidgetContainer*>(m_layout->itemAt(i)->widget()))
...
ProjectModel::INode* SoundMacroListing::currentNode() const { return m_node.get(); }

SoundMacroListing::SoundMacroListing(QWidget* parent) : QWidget(parent), m_layout(new QVBoxLayout) {
...
static const char* CategoryStrings[] = {
    ...
    QT_TRANSLATE_NOOP("SoundMacroCatalogue", "Special"), QT_TRANSLATE_NOOP("SoundMacroCatalogue", "Structure"),
...
SoundMacroCatalogue::SoundMacroCatalogue(QWidget* parent) : QTreeWidget(parent) {
  ...
  for (int i = 0; i < int(amuse::SoundMacro::CmdType::CmdTypeMAX); ++i) {
    ...
    setItemWidget(
        rootItems[i], 0,
        new CatalogueItem(amuse::SoundMacro::CmdOp::Invalid, tr(CategoryStrings[i]), tr(CategoryDocStrings[i]), this));
  }
  ...
  for (int i = 1; i < int(amuse::SoundMacro::CmdOp::CmdOpMAX); ++i) {
    ...
    if (cmd) {
      ...
      setItemWidget(
          item, 0,
          new CatalogueItem(amuse::SoundMacro::CmdOp(i), tr(cmd->m_name.data()), tr(cmd->m_description.data()), this));
    }
  }
}
...
ProjectModel::INode* SoundMacroEditor::currentNode() const { return m_listing->currentNode(); }

SoundMacroEditor::SoundMacroEditor(QWidget* parent)
: EditorWidget(parent)
, m_splitter(new QSplitter)
, m_listing(new SoundMacroListing)
, m_catalogue(new SoundMacroCatalogue) {
  {
