...
NullItemProxyModel::NullItemProxyModel(ProjectModel* source) : QIdentityProxyModel(source) { setSourceModel(source); }
...
int NullItemProxyModel::rowCount(const QModelIndex& parent) const { return QIdentityProxyModel::rowCount(parent) + 1; }
...
PageObjectProxyModel::PageObjectProxyModel(ProjectModel* source) : QIdentityProxyModel(source) {
  setSourceModel(source);
}
...
QModelIndex PageObjectProxyModel::mapFromSource(const QModelIndex& sourceIndex) const {
  ...
  auto tp = node->type();
  if ((tp != ProjectModel::INode::Type::SoundMacro && tp != ProjectModel::INode::Type::Keymap &&
       tp != ProjectModel::INode::Type::Layer && tp != ProjectModel::INode::Type::Null) ||
  ...
  case ProjectModel::INode::Type::Null:
    if (node->parent() == group)
      ...
      return createIndex(3 + smCol->childCount() + kmCol->childCount(), sourceIndex.column(),
  ...
  case ProjectModel::INode::Type::Layer:
    return createIndex(4 + smCol->childCount() + kmCol->childCount() + node->row(), sourceIndex.column(),
                       sourceIndex.internalPointer());
  default:
    break;
  }
...
QModelIndex PageObjectProxyModel::mapToSource(const QModelIndex& proxyIndex) const {
  ...
  case ProjectModel::INode::Type::Layer:
    return static_cast<ProjectModel*>(sourceModel())
        ->proxyCreateIndex(node->row() - kmCol->childCount() - smCol->childCount() - 4, proxyIndex.column(),
...
void ProjectModel::INode::_sortChildren() {
  std::sort(m_children.begin(), m_children.end(), [](const auto& a, const auto& b) { return a->name() < b->name(); });
  reindexRows(0);
}

int ProjectModel::INode::hypotheticalIndex(const QString& name) const {
  auto search =
      std::lower_bound(m_children.cbegin(), m_children.cend(), name,
                       [](const amuse::IObjToken<INode>& item, const QString& name) { return item->name() < name; });
...
int ProjectModel::GroupNode::hypotheticalIndex(const QString& name) const {
  ...
  auto search =
      std::lower_bound(m_children.cbegin(), m_children.cend() - 6, name,
                       [](const amuse::IObjToken<INode>& item, const QString& name) { return item->name() < name; });
  return int(search - m_children.cbegin());
}
  ...
  for (const auto& song : songs) {
    ...
    if (!midiData.empty()) {
      ...
      if (f.open(QFile::WriteOnly)) {
        ...
        setMIDIPathOfSong(song.second.m_setupId, m_dir.relativeFilePath(fi.filePath()));
      }
    }
  }
...
bool ProjectModel::importGroupData(const QString& groupName, const amuse::AudioGroupData& data, ImportMode mode,
                                   UIMessenger& messenger) {
  m_projectDatabase.setIdDatabases();

  amuse::AudioGroupDatabase& grp =
      *m_groups.insert(std::make_pair(groupName, std::make_unique<amuse::AudioGroupDatabase>(data))).first->second;
