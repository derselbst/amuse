...
struct SoundMacro {
  ...
  struct CmdSetNote : ICmd {
    ...
    Value<int8_t> key;
    Value<int8_t> detune;
    Seek<2, amuse::SeekOrigin::Current> seek;
    Value<bool> msSwitch;
    Value<uint16_t> ticksOrMs;
    bool Do(SoundMacroState& st, Voice& vox) const override;
  ...
  struct CmdAgeCntSpeed : ICmd {
    AT_DECL_DNA_YAMLV
    static const CmdIntrospection Introspective;
    Seek<3, amuse::SeekOrigin::Current> seek;
    Value<uint32_t> time;
    bool Do(SoundMacroState& st, Voice& vox) const override;
  ...
  void toYAML(amuse::io::YAMLDocWriter& w) const;
  void fromYAML(amuse::io::YAMLDocReader& r, size_t cmdCount);
};
...
struct AT_SPECIALIZE_PARMS(std::endian::big, std::endian::little) KeymapDNA : BigDNA {
  ...
  Seek<3, amuse::SeekOrigin::Current> pad;
};
