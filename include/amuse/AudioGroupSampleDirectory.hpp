...
constexpr uint32_t CC_RIFF = FOURCC('R', 'I', 'F', 'F');
constexpr uint32_t CC_WAVE = FOURCC('W', 'A', 'V', 'E');
constexpr uint32_t CC_FMT  = FOURCC('f', 'm', 't', ' ');
constexpr uint32_t CC_SMPL = FOURCC('s', 'm', 'p', 'l');
constexpr uint32_t CC_DATA = FOURCC('d', 'a', 't', 'a');

struct WAVHeader : LittleDNA {
  AT_DECL_DNA
  Value<uint32_t> riffMagic = CC_RIFF;
  Value<uint32_t> wavChuckSize; // everything - 8
  Value<uint32_t> wavMagic = CC_WAVE;

  Value<uint32_t> fmtMagic = CC_FMT;
  Value<uint32_t> fmtChunkSize = 16;
  WAVFormatChunk fmtChunk;

  Value<uint32_t> smplMagic = CC_SMPL;
  Value<uint32_t> smplChunkSize = 36; // 36 + numSampleLoops*24
  WAVSampleChunk smplChunk;

  Value<uint32_t> dataMagic = CC_DATA;
